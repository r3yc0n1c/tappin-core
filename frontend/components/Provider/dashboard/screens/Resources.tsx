        ...
        <div className='py-2'>
            ...
            <div className='py-4'>
                ...
                <div>
                    <div className='py-2 flex gap-2 flex-wrap'>
                        {ram.map((ele, idx) => {
                            return <Button className={twMerge(selectedRAM === idx ? "bg-green-300 text-black" : "", "offsetstyle generalTabsBorder")} onClick={() => {
                                setSelectedRAM(idx)
                            }} key={idx}>{ele}</Button>
                        })}
                    </div>
            ...
            <div className='py-2'>
                ...
                <div>
                    <div className='py-2 flex gap-2 flex-wrap'>
                        {cores.map((ele, idx) => {
                            return <Button className={twMerge(selectedCore === idx ? "bg-yellow-300 text-black" : "", "offsetstyle generalTabsBorder")} onClick={() => {
                                setSelectedCore(idx)
                            }} key={idx}>{ele}</Button>
                        })}
                    </div>
            ...
            <div className='flex flex-col'>
                ...
                <div className='py-2 flex flex-col offsetEffect generalBorder my-2 w-fit bg-yellow-300'>
                    <span className='text-2xl font-semibold'>
                        {returnPrice(selectedCore, selectedRAM)[0]} USD $ <span className='text-xs text-gray-500'>/hr</span>
                    </span>
                    <span className='text-2xl font-semibold'>
                        {returnPrice(selectedCore, selectedRAM)[1]} SOL <span className='text-xs text-gray-500'>/hr</span>
                    </span>
                </div>
            </div>
        </div>
